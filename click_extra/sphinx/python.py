...
MIRROR_SRC_MARKER_END = "<!-- mirror-src-end -->"
"""Closing marker of a `<!-- mirror-src … -->` region.

The comment form of `:mirror:`: the generator Python lives inside an
`<!-- mirror-src … -->` HTML comment instead of a visible `{python:render}`
fence, so the generated body renders on GitHub or PyPI (a README diagram, a
compatibility table) while the generator stays invisible. Everything between the
opening comment's closing `-->` and this marker is regenerated by
{func}`update_mirror_blocks`; edit the comment, never the body.
"""

# Opening `<!-- mirror-src -->` comment line, the generator body's closing `-->`,
# and the `<!-- mirror-src-end -->` region terminator.
_MIRROR_SRC_OPEN_RE = re.compile(r"^[ \t]*<!--[ \t]*mirror-src[ \t]*$")
_MIRROR_SRC_CODE_CLOSE_RE = re.compile(r"^[ \t]*-->[ \t]*$")
_MIRROR_SRC_END_RE = re.compile(r"^[ \t]*<!--[ \t]*mirror-src-end[ \t]*-->[ \t]*$")
...
def _rewrite_mirror_src_regions(text: str, location: str) -> str:
    """Return `text` with every `<!-- mirror-src … -->` region refreshed.

    The comment counterpart of {func}`_rewrite_mirror_regions`: instead of a
    visible `{python:render}` `:mirror:` fence, the generator Python lives inside
    an `<!-- mirror-src -->` HTML comment, so the generated body renders on
    GitHub or PyPI while the generator stays invisible. The body runs through
    {func}`_execute_mirror_block` (so a sibling helper module imports the same
    ...
    while index < total:
        ...
        # Copy the opening comment and generator body verbatim, up to `-->`.
        out.append(lines[index])
        index += 1
        body: list[str] = []
        while index < total and not _MIRROR_SRC_CODE_CLOSE_RE.match(lines[index]):
            body.append(lines[index])
            out.append(lines[index])
            index += 1
        if index >= total:
            # Unterminated comment: leave the tail untouched.
            break
        out.append(lines[index])  # the `-->` line
        index += 1

        generated = _execute_mirror_block(body, namespace, location)

        # Find any existing generated body, up to the region terminator.
        end = index
        while end < total and not _MIRROR_SRC_END_RE.match(lines[end]):
            end += 1

        out.extend(["", *generated, ""])
        if end < total:
            # Replace the existing body, consume through the terminator, and
            # collapse the gap to the following content to a single blank line.
            ...
            # First sight: insert a fresh region; following content stays put.
            out.append(MIRROR_SRC_MARKER_END)
...
def update_mirror_blocks(paths: Iterable[Path], *, check: bool = False) -> list[Path]:
    ...
    def rewrite(text: str, path: Path) -> str:
        location = str(path)
        return _rewrite_mirror_src_regions(
            _rewrite_mirror_regions(text, location), location
        )
...
def rewrite_python_mirror_regions(
    ...
    Applies {func}`_rewrite_mirror_regions` (the visible `{python:render}`
    `:mirror:` fence) and {func}`_rewrite_mirror_src_regions` (the invisible
    `<!-- mirror-src -->` comment) to the in-memory `source` before the document
    is parsed, so every build renders the fresh output even when the committed
    region is stale. Nothing is written to disk: the committed region is
    refreshed offline by {func}`update_mirror_blocks`, behind the
    ...
    """
    text = source[0]
    has_fence = "python:render" in text and ":mirror:" in text
    has_src = "<!-- mirror-src" in text
    if not (has_fence or has_src):
        return
    location = str(app.env.doc2path(docname))
    if has_fence:
        text = _rewrite_mirror_regions(text, location)
    if has_src:
        text = _rewrite_mirror_src_regions(text, location)
    source[0] = text
