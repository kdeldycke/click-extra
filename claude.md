...
**The coupling reaches the CLI surface too, not only imported symbols.** repomatic's workflows shell out to this package's own commands, so a renamed flag breaks a release job that no local gate and no test in this repository executes. `9.0.0` moved roff output from `wrap --man` to `wrap --help-format man`, and the release engine's `manpages` job still passed the old pair: the run went green on everything else and the version shipped **permanently without its man-pages asset**, since publishing locks a release's assets. So when a breaking change touches a CLI option, grep `../repomatic` for the old spelling as well as this tree, and treat the fix as part of the same change rather than a follow-up. A docs sweep does not cover it: the eight stale `--man` examples in `docs/` were all corrected in that release while the workflow calling the same command was not.
