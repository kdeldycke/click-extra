...
- New `click_extra.styling` module exposes `fields_to_dict`, `dict_to_fields`, and `cascade_fields` — shared dataclass-roundtrip helpers used by both `Style` and `HelpExtraTheme` for `to_dict` / `from_dict` / `cascade` machinery.
...
- Fix `reset_loggers` double-registration: when both `--verbosity` and `-v` are passed, the close callback is now registered at most once per invocation via a sentinel in `ctx.meta`.
