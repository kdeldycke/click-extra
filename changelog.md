...
- Refactor the theme system around plain `HelpExtraTheme` instances instead of staticmethod factories on the dataclass. Built-in themes (`DARK`, `LIGHT`) and the `BUILTIN_THEMES` mapping now live in a new `click_extra.themes` module: adding a theme is one constant in that file. `HelpExtraTheme.dark()` and `HelpExtraTheme.light()` are removed. `register_theme()` and `theme_registry` accept either a `HelpExtraTheme` instance or a zero-argument callable returning one, with callables resolved at `--theme` parse time for cases that need runtime detection. `default_theme` is now `click_extra.themes.DARK`. **Breaking:** `HelpExtraTheme.dark()` / `.light()` callers must replace those calls with the `DARK` / `LIGHT` instances from `click_extra` or `click_extra.themes`.
- Add four hand-rolled branded themes to `click_extra.themes`: `solarized_dark` (Ethan Schoonover), `dracula` (Zeno Rocha), `nord` (Arctic Ice Studio), and `monokai` (Wimer Hazenberg). Each ships its canonical 24-bit RGB palette mapped slot-by-slot onto Click Extra's help-screen slots. The mapping is hand-curated because generic colour-scheme catalogs (base16, pygments, iTerm palettes) don't expose the same semantic roles (option, metavar, choice, deprecated, envvar, ...) and produce poor mechanical translations. The `--theme` choice list now reads `[dark|dracula|light|monokai|nord|solarized_dark]`.
