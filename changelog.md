...
- Add the always-on `matrix:python` Sphinx directive to `click_extra.sphinx`, which renders a package's Python-version compatibility matrix. The table is embedded in the directive block and rendered without git access at build time; it accepts `:package:`, `:path:`, `:python-floor:`, `:version-floor:`, and `:tag-pattern:` options, and needs no `click_extra_enable_exec_directives` opt-in.
- Add the `click-extra refresh-directives` command (and the `click_extra.sphinx.matrix.update_matrix_blocks` function it wraps) to regenerate the tables embedded in `matrix:python` blocks from a project's git tags. It refreshes only existing blocks, fails gracefully when the sphinx extra is absent, and its `--check` mode flags stale tables in CI.
