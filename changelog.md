...
- Add a `:mirror:` flag to the `python:render` Sphinx directive. On top of rendering live, a mirror block writes its generated Markdown back into the source `.md` between HTML-comment markers directly below the fence, rewritten on every build, so the document self-updates and its output is reviewable in the raw file, in diffs, and on GitHub.
