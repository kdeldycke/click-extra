...
- Default the test-plan config file (`TestPlanConfig.file`) to `./tests/cli-test-plan.toml` instead of `./tests/cli-test-plan.yaml`. TOML parses with the built-in `tomllib`, so the default no longer requires the optional `[yaml]` extra to work. Click Extra's own dogfooded test plan is now written in TOML.
- Allow declaring test-plan cases natively in the config file under `[tool.<cli>.test-plan.cases]` (an array of tables in TOML), as an alternative to a `file` path or an `inline` YAML string. Adds the `cases` field to `TestPlanConfig` and the public `cases_from_data` helper, the in-memory counterpart to `parse_test_plan` and `load_test_plan`.
