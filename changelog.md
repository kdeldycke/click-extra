...
- New `click_extra.themes` module hosts the built-in theme catalog. Each theme is a `HelpExtraTheme` subclass (`Dark`, `Light`, `SolarizedDark`, `Dracula`, `Nord`, `Monokai`) whose field defaults declare its palette: Sphinx autodoc renders each one as a class with a clean attribute table (matching the `LogLevel` / `TableFormat` rendering), and subclassing for tweaks (`class MyDark(Dark): warning = Style(...)`) is straightforward. Each subclass also exposes a cached singleton with the matching UPPER_CASE name (`DARK`, `LIGHT`, ...) for direct use. Adding a new theme is a one-file edit in `themes.py`: declare the subclass, instantiate the singleton, register it in `BUILTIN_THEMES`. New `tests/test_themes.py` (5 checks) enforces alphabetical ordering of classes, singletons, and `BUILTIN_THEMES` keys.
