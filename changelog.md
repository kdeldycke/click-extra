...
- Add a `click_extra_manpages` Sphinx configuration value (and the matching `click_extra.sphinx.manpages` module). Projects that already enable the `click_extra.sphinx` extension can declare one or more `{"script", "prog_name", "output_dir"}` entries to emit the corresponding Click command tree(s) as roff `.1` files into `<outdir>/<output_dir>/` on every HTML build. Each `script` is resolved with the same scanner the `click-extra man` CLI uses (console-script entry point, `module:function`, `.py` path, or module name); only HTML-family builders (`html`, `dirhtml`, `singlehtml`) trigger the hook. The hook stays silent when the config is empty or absent.
