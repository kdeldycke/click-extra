...
- Extend the `--show-params` table with seven new columns (`Is flag`, `Flag value`, `Is bool flag`, `Multiple`, `Nargs`, `Prompt`, `Confirmation prompt`) and convert `ShowParamsOption.TABLE_HEADERS` to a tuple of `ColumnSpec` frozen dataclasses carrying a stable `id`, a display `label`, and a MyST `description`. The *Available columns* section of the docs is now auto-generated at Sphinx build time from this single source of truth, via a `show_params_columns_table` MyST substitution. Add the `--columns` option (`ColumnsOption`, `@columns_option`) for SQL `SELECT`-style column selection on both the integrated `--show-params` flag and the standalone `click-extra show-params` subcommand: `--columns id,spec,value` keeps the user-given order and rejects unknown IDs at parse time. The `select_columns()` and `select_row()` helpers expose the projection logic for downstream consumers. The standalone subcommand also drops three runtime/config-dependent columns (`Allowed in conf?`, `Value`, `Source`) from its defaults; it was previously emitting 15-cell rows under 8 headers and rendering misaligned output.
- Add `MultiChoice` to `click_extra.types`: a generic Click `ParamType` for comma-separated SQL `SELECT`-style multi-pick from a fixed set of values. The pick-many counterpart to `click.Choice`. Accepts a single token, validates each value via `self.fail()` when `choices` is set, normalizes case when `case_sensitive=False`, and renders a `[a,b,c]` metavar parallel to `click.Choice`'s `[a|b|c]`. The help-screen colorizer detects both pipe- and comma-separated metavars and highlights each value individually. `ColumnsOption` is the headline consumer; the type is also exposed for arbitrary tags / categories / modes use cases. Click does not ship an equivalent: the upstream conversation is in [pallets/click#2771](https://github.com/pallets/click/issues/2771) (open, exactly this feature request) and [pallets/click#2537](https://github.com/pallets/click/issues/2537) (closed as not planned).
...
- **Breaking:** `theme_registry` no longer accepts a `Callable[[], HelpExtraTheme]` value; only `HelpExtraTheme` instances are valid entries. The runtime-detection-via-callable pattern was documented but never used; loading themes from `--config` is the supported equivalent (see [Themes from your config file](https://kdeldycke.github.io/click-extra/theme.html#themes-from-your-config-file)).
