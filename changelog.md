...
- Speed up `import click_extra` by deferring the `requests`, `tabulate`, PyYAML and `boltons.ecoutils` imports until first use, and replacing the runtime `unittest.mock` dependency with a small built-in context manager. None of these load on the default import path anymore, cutting startup time by roughly a third.
