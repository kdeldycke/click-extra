...
## [`8.1.0.dev0` (unreleased)](https://github.com/kdeldycke/click-extra/compare/v8.0.1...main)
...
- **Breaking:** Remove the `inline` field from the test-suite config. Declare cases natively under `[tool.<cli>.test-suite.cases]`, or set `file` to a suite path, instead.
- Add a [Carapace](https://carapace.sh) completion spec exporter. The new `click_extra.carapace` module walks a Click command tree and serializes it to the [carapace-spec](https://github.com/carapace-sh/carapace-spec) YAML format, giving any CLI native completion across every shell Carapace supports (Bash, Zsh, Fish, Nushell, PowerShell, Elvish, and more). Answers [`click#3188`](https://github.com/pallets/click/issues/3188). Adds the `to_carapace_spec`, `dump_carapace_spec`, `write_carapace_spec` and `install_carapace_spec` API, behind a new `carapace` extra (PyYAML).
- Choices, file and directory operands, and the command hierarchy are inlined into the spec for zero-latency completion; a parameter with a custom `shell_complete` instead emits a callback action handled by a new `carapace` shell completion class, registered on `import click_extra`. A click-extra command's default options are exported as Carapace `persistentflags` on the root.
...
- The generated spec's header comment now records the command that produced it (`# $ click-extra wrap --carapace -- <target>`) and a link to the documentation, alongside the generator-version line. `dump_carapace_spec`, `write_carapace_spec` and `install_carapace_spec` gain an `invocation` keyword to set that line.
- The `wrap` command now accepts a local project directory as its target: its console-script entry point is read from `pyproject.toml` (`[project.scripts]`) or `setup.cfg` (`console_scripts`), and the directory holding its top-level package is prepended to `sys.path`, so a checked-out project can be wrapped by path without installing it first. Both the flat and src layouts are detected.
...
- Man pages now render optional-value options like `--color` as `--color[=auto|always|never]` (a bare flag is allowed) instead of as a required value, and drop the spurious value metavar on repeatable count options like `-v`/`--verbose`. The man-page and Carapace exporters derive an option's value shape from shared helpers in `click_extra.parameters` (`option_value_kind`, `is_repeatable`, `param_spellings`, `short_long_opts`); `full_short_help` moved there from `click_extra.man_page`.
- Add `iter_subcommands` and `make_resilient_context` to `click_extra.parameters`, a single source for enumerating a group's visible subcommands and building a parse-free introspection context. The man-page and Carapace exporters share them instead of each re-implementing the same `list_commands` / `get_command` / `make_context` traversal.
- Unify the "missing optional dependency" error messages behind a shared `missing_extra_message` helper in `click_extra.parameters`, used by the documentation integrations (mkdocs, pygments, sphinx, pytest), the Carapace exporter and the table formatters. The four module gates now point at the canonical hyphenated `pip install click-extra[<extra>]` target (they previously used the non-canonical `click_extra[<extra>]` form) with consistent wording.
...
- Command and option decorators built by `decorator_factory` now carry explicit overloaded type signatures, so static type checkers (basedpyright, pyright, mypy) infer the produced command or the decorated callback instead of flagging the decorators as untyped. This also covers the optional-parenthesis form: a bare `@group` now resolves to `Group` rather than `Callable[[], None]`. Closes [`click-extra#1781`](https://github.com/kdeldycke/click-extra/issues/1781).
...
- Restrict pytest collection to the `tests` folder and switch to `importlib` import mode, so the `test_suite.py` module is no longer collected as a test file. Fixes `import file mismatch` errors when building from a packaged source tree alongside an installed copy. Closes [`click-extra#1779`](https://github.com/kdeldycke/click-extra/issues/1779).
- `test-suite --suite-file` now accepts a suite in any list-capable config format, detected from the file extension: TOML and JSON (built-in), plus YAML, JSON5, JSONC and Hjson (with their extras). A TOML suite lists its cases under a `[[cases]]` array of tables. Adds the `load_test_suite` helper and the `SUITE_FORMATS` constant.
- Accept an integer `timeout` in a test suite case: a bare number like `timeout: 5` is coerced to a float instead of being rejected.
...
- Allow declaring test-suite cases natively in the config file under `[tool.<cli>.test-suite.cases]` (an array of tables in TOML), as an alternative to a `file` path. Adds the `cases` field to `TestSuiteConfig` and the public `cases_from_data` helper, the in-memory counterpart to `parse_test_suite` and `load_test_suite`.
