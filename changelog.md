...
- **Breaking:** Split three large modules into focused new ones, each now the single home for its symbols (no compatibility re-exports from the original module): the config schema and validation engine into `click_extra.config_schema` (`ValidationError`, `ValidationReport`, `ConfigValidator`, `make_schema_callable`, `run_config_validation`, `normalize_config_keys`, `flatten_config_keys`, `get_tool_config`, the dotted-key helpers), the theme palette and HTML documentation renderer into `click_extra.theme_docs` (`palette_html`, `inject_slot_example_docstring`), and the spinner preset catalog into `click_extra.spinner_presets` (`SPINNERS`, `SpinnerPreset`, the frame constants). The public names still re-exported at the `click_extra` package root are unchanged; update any deeper imports like `from click_extra.config import ValidationError` to the new module.
