...
- Reverse `{matrix}` column order on both axes: newest Python and dependency versions now sit on the left, so the most recent compatibility information reads from the upper-left corner.
