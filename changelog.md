...
## [`8.5.0.dev0` (unreleased)](https://github.com/kdeldycke/click-extra/compare/v8.4.0...main)
...
- Promote `ctx.render_table` and `ctx.print_table` to real typed methods on `click_extra.Context`, reading the `--table-format` selection and the `--sort-by` sort key from the context's shared `meta` (new `TABLE_SORT_KEY` entry). They now work from any subcommand without reaching for the root context, and without a table option in the chain they fall back to the default format. `TableFormatOption` and `SortByOption` keep injecting bound instance attributes on plain `click.Context` instances, so both options remain drop-ins on foreign click and cloup commands. `ctx.render_table` now honors `--sort-by`, like `ctx.print_table` always did.
- Warn on configuration keys unknown to the `config_schema` when the section is schema-only (`included_params=()`), instead of dropping them silently: any such key can only be a typo. Strict mode still raises; sections mixing CLI parameters stay silent. Exposed as `warn_unknown` on `make_schema_callable()` and `schema_warn_unknown` on `run_config_validation()`.
- Add the `click_extra.sphinx.myst_docstrings` Sphinx extension: write Python docstrings in MyST markdown, transparently converted to reST at build time through `autodoc-process-docstring`, keeping `sphinx.ext.autodoc` and `sphinx_autodoc_typehints` unmodified. Absorbed from `repomatic.myst_docstrings`.
- Add a `convert-to-myst` subcommand (`click-extra convert-to-myst`) migrating reST docstrings to MyST in place, idempotently, auto-detecting the source package from the project's script entry points. Its `click_extra.myst_converter` machinery and the `sphinx-apidoc` stub converter (`click_extra.rst_to_myst`) are absorbed from `repomatic`.
- Restrict `convert-to-myst` rewrites to docstrings and comments: string literals, f-strings, and other runtime code now pass through byte-for-byte. Directives nested inside a fence, directives whose body holds a fence, and domain-qualified roles (like `` :py:class:`X` ``) stay in reST so re-runs are stable and fences never nest at the same level. Adjacent brace-bearing literals no longer get their delimiters halved across the gap separating them. The per-module conversion is exposed as `convert_source()`.
- Fix the `myst_docstrings` build-time conversion mangling reST double-backtick literals whose content ends on a brace (like ``` ``{levelname}:{message}`` ```), and code spans holding a literal role or option name (like `` `:tag-pattern:` ``).
...
- Add a `themes` demo subcommand (`click-extra themes`) that prints a sample help screen rendered under every built-in theme, one after another, for a quick terminal preview of all palettes.
- Fix a group-level `--theme` not reaching the CLI wrapped by `click-extra wrap`: `click-extra --theme <name> -- <cli> --help` now renders the target's help in the chosen palette instead of always falling back to the default `dark` theme.
- Add a `:mirror:` flag to the `python:render` Sphinx directive. On top of rendering live, a mirror block keeps a copy of its generated Markdown in the source `.md`, between `<!-- mirror -->` and `<!-- mirror-end -->` markers directly below the fence, so its output is reviewable in the raw file, in diffs, and on GitHub. Sphinx builds regenerate the region in memory and never write to the source.
...
- Fix `click-extra refresh-directives` rewriting `{matrix}` examples nested inside longer code fences: documented illustrations are now copied verbatim, never refreshed or executed.
