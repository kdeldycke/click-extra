...
- The built-in theme catalog (`dark`, `dracula`, `light`, `monokai`, `nord`, `solarized_dark`) now ships as the package data file `click_extra/themes.toml` instead of Python code. `BUILTIN_THEMES` and the `DARK` / `DRACULA` / `LIGHT` / `MONOKAI` / `NORD` / `SOLARIZED_DARK` constants live in `click_extra.theme` and are loaded from the TOML at import time via `HelpExtraTheme.from_dict`. Adding a built-in theme is a one-file data edit: declare a `[<name>]` table with one inline-table per styled slot. Tests parse `themes.toml` directly to enforce alphabetical ordering and that every TOML payload round-trips through `to_dict`. **Breaking:** the `click_extra.themes` module is gone; import from `click_extra` (canonical path) or `click_extra.theme` instead. Deriving a tweaked palette stays a one-liner via `DARK.with_(option=Style(...))`.
- New `HelpExtraTheme.to_dict()` / `HelpExtraTheme.from_dict()` round-trip a theme through plain mappings, so a theme can live in a TOML/JSON/YAML configuration file alongside the rest of an application's settings. `to_dict()` emits only slots that diverge from the default; `from_dict()` raises `TypeError` on unknown keys so configuration typos surface at load time.
- `--config` now recognizes a `themes` sub-table in the app's section: every `[<cli>.themes.<name>]` (or `[tool.<cli>.themes.<name>]` in `pyproject.toml`) is parsed via `HelpExtraTheme.from_dict` and made available to `--theme` for the current invocation. Known names cascade on top of the matching built-in palette through the new `HelpExtraTheme.cascade(base)` method (sparse overlay: slots set in the config win, unset slots inherit); unknown names become stand-alone themes. The registry mutation lives on `ctx.meta` under `click_extra.context.THEME_OVERRIDES` and never touches the module-level `theme_registry`, so back-to-back invocations in the same process don't cross-contaminate. A built-in `ConfigValidator` for the `themes` extension path is auto-registered on every `ConfigOption`, so malformed entries surface as `ValidationError` with a rooted path during `--validate-config` and at normal load time. `wrap` inherits the same behavior through `default_extra_params()`.
- New `ThemeChoice` param type backs `--theme`. Where `click.Choice` snapshots its choice list at instantiation, `ThemeChoice.choices` is a live view: it reads `theme_registry` plus any per-context overrides at every lookup, so themes loaded from `--config` appear as valid choices and in the `--help` metavar without any further wiring. Subclasses `click.Choice` to preserve the per-choice token coloring `click_extra.colorize` already applies to choice metavars.
- Reorder `default_extra_params()` so `--config` / `--no-config` / `--validate-config` precede `--color` / `--theme`, matching the declaration order described in the docstring and ensuring config-loaded themes are visible to `--theme`'s validation. The new declaration order also makes the visible order in `--help` match the documented processing order.
- Add four branded themes pulled from each scheme's canonical palette: `solarized_dark` (Ethan Schoonover), `dracula` (Zeno Rocha), `nord` (Arctic Ice Studio), `monokai` (Wimer Hazenberg). Slot mappings are hand-curated: generic colour-scheme catalogs (base16, pygments, iTerm) don't expose the same semantic roles (option, metavar, choice, deprecated, envvar, ...) and produce poor mechanical translations. The `--theme` choice list now reads `[dark|dracula|light|monokai|nord|solarized_dark]`.
- Promote `--theme` from a `wrap`-only flag to a member of `default_extra_params()`. Every command/group built with `@click_extra.command` / `@click_extra.group` now accepts `--theme [dark|...]`, reads its value from `[tool.<cli>]` config sections, and stores the chosen theme on `ctx.meta` (key `click_extra.context.THEME`) for subcommand inheritance. The `wrap` subcommand drops its local `--theme` and reads the value from the parent group's context.
- New `click_extra.theme` module centralizes the theme machinery: `HelpExtraTheme`, `default_theme`, `nocolor_theme`, `OK`, `KO`, `ThemeOption`, `theme_option` decorator, `theme_registry`, `register_theme()`, and `get_current_theme()`. `register_theme()` and `theme_registry` accept either a `HelpExtraTheme` instance or a zero-argument callable returning one (callables resolved at `--theme` parse time for runtime-detected themes). **Breaking:** `HelpExtraTheme.dark()` / `.light()` are removed: replace with the `DARK` / `LIGHT` instances exported from `click_extra` or `click_extra.themes`. Code importing theme symbols directly from `click_extra.colorize` must update to `click_extra.theme` or `click_extra.themes`.
- New `click_extra.styling` module ships a feature-rich `Style` subclass of `cloup.Style`, transparently shadowing it as `from click_extra import Style`. Eight additions on top of cloup's contract: compact single-line `__repr__` (hides `None`/falsy fields, RGB tuples render as `#rrggbb`); hex shorthand on the constructor (`Style(fg="#f1fa8c")`, including 3-digit `#abc`); `__str__` returning a styled `"sample"` for REPL/debug visualization; composition operator `a | b` (right operand wins on conflicts; reflected `__ror__` so `cloup.Style | click_extra.Style` works); `cascade(base)` for theme-inheritance fills; `to_dict()` / `from_dict()` for TOML/JSON round-trip; `to_css()` emitting CSS declaration lists for HTML rendering; `from_ansi()` parsing one or more consecutive SGR escapes back into a `Style` (8/16-color, 256-color, 24-bit); `contrast_ratio(other)` computing the WCAG 2.x luminance ratio between two foregrounds. Equality and hash override cloup's defaults to ignore the lazy `_style_kwargs` cache so identical styles compare equal regardless of call history. New `tests/test_styling.py` covers the additions with 57 tests.
...
- Fix pyproject.toml CWD discovery to skip files lacking a `[tool.<cli_name>]` section. An unrelated `pyproject.toml` (like a dotfiles repo's `[tool.ruff]`) no longer shadows the user's app-dir config: the walk continues upward to look for a matching `[tool.<cli_name>]` section, and falls back to the standard app-dir search if none is found.
- Extend configuration validation with an app-defined hook for sub-trees whose keys are *data* rather than CLI flag names (per-plugin overrides, matrix axes, user-defined IDs). New public types: `ConfigValidator` (binds a dotted `extension_path` to a callable that inspects the matching sub-tree), `ValidationError` (single exception shape carrying a rooted `path`, `message`, and optional `code`), and the `EXTENSION_METADATA_KEY` dataclass-field flag for marking extension points whose Python type isn't a mapping. Validators are registered through a new `config_validators=` kwarg on `@command` / `@group` / `@config_option`, run during both `--validate-config` and normal `--config` loading, and receive their sub-tree pre-extracted and rooted at the app section. Schema-typed `dict[str, X]` fields are also recognized as extension points without an explicit marker. Click-extra's strict check and dataclass schema both skip those sub-trees, so user-controlled tables no longer trip unknown-key detection. `--validate-config` now collects every error before exiting (CLI-flag strict-check, schema processing, and validator failures) and surfaces them with the same path-rooted `ValidationError` shape regardless of which layer detected the problem.
