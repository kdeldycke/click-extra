...
## [`8.0.0.dev0` (unreleased)](https://github.com/kdeldycke/click-extra/compare/v7.20.1...main)
...
- **Breaking:** Drop the `Extra` prefix from public classes, so the bare name now resolves to click-extra's enhanced version like `Option`, `Argument` and `Style` already did: `ExtraCommand` to `Command`, `ExtraGroup` to `Group`, `ExtraContext` to `Context`, `ExtraVersionOption` to `VersionOption`, `HelpExtraFormatter` to `HelpFormatter`, `HelpExtraTheme` to `HelpTheme`, `ExtraFormatter` to `Formatter`, `ExtraStreamHandler` to `StreamHandler`, `extraBasicConfig` to `basicConfig`, `new_extra_logger` to `new_logger` and `default_extra_params` to `default_params`. `ExtraCliRunner` is now the top-level `CliRunner`, and its result object is exposed as `Result`. `ExtraOption` keeps its prefix on purpose: it marks click-extra's own built-in options, and `Option` is already the user-facing wrapper.
...
- Fix the `assert_output_regex` testing fixture raising a `TypeError` on a failed match under pytest 9, where a private pytest helper it relied on changed signature. The mismatch diff is now built with the standard-library `difflib`.
- Speed up `import click_extra` by deferring the `tabulate`, PyYAML and `boltons.ecoutils` imports until first use, and replacing the runtime `unittest.mock` dependency with a small built-in context manager. None of these load on the default import path anymore, cutting startup time by roughly a third.
- Drop `requests` as a runtime dependency. The configuration-from-URL loader now uses the standard-library `urllib.request`, which keeps the feature working out of the box without pulling `requests` and its urllib3 / charset-normalizer / idna stack into every install.
...
- Under `auto`, a `dumb` or `unknown` `TERM` now strips ANSI color from help and output even on a terminal, matching how the spinner already suppresses its animation there and aligning with Rich. An explicit `--color`/`FORCE_COLOR` still wins.
- Branded 24-bit themes (`dracula`, `monokai`, `nord`, `solarized_dark`) downsample their colors to the nearest 256-color index when the terminal does not advertise truecolor, instead of always emitting 24-bit and relying on the terminal to convert. Detection is optimistic: 24-bit is kept unless `COLORTERM` is set to an explicit non-truecolor value or `TERM` ends in `-16color`.
...
- Add a `-q`/`--quiet` option (`QuietOption`, `@quiet_option`) to the default option set: the symmetric counterpart of `-v`/`--verbose`. Each `-q` lowers the log level one step toward `CRITICAL`, while `-v` raises it toward `DEBUG`; the two form a signed counter that cancels out (`-v -q` is a no-op) and reconciles with `--verbosity LEVEL`. The raw count is published at `ctx.meta["click_extra.quiet"]`.
- Add `click_extra.progressbar`, a drop-in for `click.progressbar` that hides the determinate bar under `--no-progress` or `--accessible`, so it follows the same flag as the indeterminate spinner. An explicit `hidden=` argument still forces the bar either way; its color stays governed by `--no-color`/`NO_COLOR`.
- Under `--accessible`, `click_extra.clear` becomes a no-op and `click_extra.echo_via_pager` streams its text straight to stdout instead of spawning a pager. Both publish off the new `ctx.meta["click_extra.accessible"]` flag and defer to their Click counterparts otherwise.
...
- Add `{git_distance}` and `{git_dirty}` version-string template variables: the number of commits since the most recent tag, and whether the work tree has uncommitted changes (`dirty`/`clean`). Both support pre-baking via `__git_distance__`/`__git_dirty__` dunders and `click-extra prebake all`.
- `VersionOption` git fields fall back to a `.git_archival.json` file (the setuptools-scm and Dunamai schema) when a CLI runs from a `git archive` export with no `.git` directory, such as a GitHub source tarball. Commit a `.git_archival.json` and a `.gitattributes` marking it `export-subst` to populate the hash, date, branch, tag and distance fields in archives.
...
- Re-type `pass_context` for the enhanced `Context`, so `@pass_context def cmd(ctx: Context)` type-checks (Click's own is typed for the base `click.Context`, rejecting the subclass by parameter contravariance); handlers typed for the base `click.Context` still work.
- Add a declarative CLI test-plan engine (`click_extra.test_plan`): `CLITestCase`, `parse_test_plan`, and `run_test_plan` run a YAML plan of invocations against any command or binary as subprocesses, checking exit codes and output, parallelized via `run_jobs`. The black-box complement to the in-process `CliRunner`.
- Add a `click-extra test-plan` subcommand running a plan against a `--command` or binary, with `--plan-file`, `--plan-envvar`, `--select-test`, `--skip-platform`, `--jobs`, and `--exit-on-error`. YAML parsing needs the `yaml` extra.
- `click-extra test-plan` resolves its plan from `[tool.click-extra.test-plan]` config (`file`, `inline`, `timeout`) when no plan is passed on the command line, so a project can configure its plan once via the `TestPlanConfig`/`ClickExtraConfig` schema.
...
- Tolerate a missing `click_extra/themes.toml` data file instead of aborting at import: log a warning, fall back to the no-color theme, and make `--theme` inert when no built-in or config themes are available. Keeps CLIs usable when a packaging or distribution step drops the bundled data file.
- Fix the `show_choices` context setting being ignored: passing `context_settings={"show_choices": True}` (or `False`) to a `Command` or `Group` now forces that value on every option, instead of silently doing nothing and overwriting each option's `show_envvar`.
- **Breaking:** Reorganized the configuration subsystem into a `click_extra.config` package, split by role into three submodules: `click_extra.config.schema` (the schema and validation engine: `ValidationError`, `ValidationReport`, `ConfigValidator`, `make_schema_callable`, `run_config_validation`, `normalize_config_keys`, `flatten_config_keys`, `get_tool_config`, the dotted-key helpers), `click_extra.config.formats` (the `ConfigFormat` registry and per-format parsers), and `click_extra.config.option` (`ConfigOption`, `NoConfigOption`, `ValidateConfigOption`, and the `ClickExtraConfig`/`TestPlanConfig`/`PrebakeConfig` schemas). Every config symbol stays importable from `click_extra.config`, so `from click_extra.config import ValidationError` keeps working.
- **Breaking:** Split two more large modules into focused new homes, each now the single home for its symbols with no compatibility re-exports from the original module: the theme palette and HTML documentation renderer into `click_extra.theme_docs` (`palette_html`, `inject_slot_example_docstring`), and the spinner preset catalog into `click_extra.spinner_presets` (`SPINNERS`, `SpinnerPreset`, the frame constants). The public names re-exported at the `click_extra` package root are unchanged; update any deeper imports to these new modules.
- **Breaking:** Removed the dead, undocumented `ParamStructure.flatten_tree_dict` and `_flatten_tree_dict_gen` methods (a tree-dict flattener left with no callers after earlier refactors).
- **Breaking:** Split the old `click_extra.colorize` module in two. The help-screen highlighting engine (`HelpFormatter`, `HelpKeywords`, `highlight`, and the internal `_HelpColorsMixin`/`_escape_for_help_screen`) moves to a new `click_extra.highlight` module. The remaining color-mode resolution (`ColorOption`, `NoColorOption`, `forced_color`, `resolve_color_env`, the `FORCE_COLOR`/`NO_COLOR` env handling and the tri-state `--color[=WHEN]` logic) is renamed to `click_extra.color`, since it no longer applies color but only decides whether color should be applied. The public names re-exported at the `click_extra` package root are unchanged; update deeper imports: `from click_extra.colorize import HelpFormatter` becomes `from click_extra.highlight import HelpFormatter`, and `from click_extra.colorize import ColorOption` becomes `from click_extra.color import ColorOption`.
- **Breaking:** Remove `print_sorted_table`. `SortByOption` now bakes the resolved row sort key into `ctx.print_table`, so the same `ctx.print_table(table_data, headers)` call applies whether or not a `--sort-by` option is present; previously the sorted variant silently swapped the first two positional arguments to `(header_defs, table_data)`.
- **Breaking:** Remove the unused `click_extra.styling.cascade_fields` helper.
- Add a `click_extra.prebake` module hosting the build-time version pre-baking helpers (`prebake_version`, `prebake_dunder`, `discover_package_init_files`), split out of `click_extra.version`; they stay importable from `click_extra.version`. The `version` module now focuses on the runtime `VersionOption` and the git/archival metadata it reads.
- `click-extra man` and `write_manpages` resolve the package author and version through the same metadata logic as `--version`, fixing author display-name parsing and distribution-name resolution for packages whose import name differs from the installed distribution name.
- **Breaking:** Drop the dead `ParamStructure.__init__` and the redundant `ParamStructure.SEP` class attribute; use the module-level `click_extra.parameters.PARAM_PATH_SEP` instead. Both were superseded when the two parameter-tree walkers converged onto the single `walk_command_params`.
- **Breaking:** Stop re-exporting the private config helpers (`_builtin_config_validators`, `_collect_opaque_paths_from_schema`, `_expand_dotted_keys`, `_select_app_section`, `_strip_opaque_subtrees`, `_strip_reserved_keys`) through `click_extra.config`; import them from `click_extra.config.schema` if needed.
- `--validate-config` now raises `RuntimeError` instead of `TypeError` when used on a command with no sibling `--config` option, matching the error `--no-config` already raised in the same situation.
