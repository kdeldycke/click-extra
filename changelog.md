...
## [`7.21.0.dev0` (unreleased)](https://github.com/kdeldycke/click-extra/compare/v7.20.1...main)
...
- Add a `Spinner` widget: an indeterminate, thread-animated terminal spinner for long-running blocking work where `click.progressbar` does not fit. Auto-disabled when its output stream is not a TTY, and enables Windows console virtual-terminal processing on start so its ANSI codes animate rather than print literally. Usable as a context manager or a decorator (with or without parentheses), with a live-updatable label, custom frames, reverse rotation, `Style`-based coloring, an opt-in terminal-bell `beep` on stop, an elapsed-time `timer` (with an optional custom-format callable), an `echo()` method to print above the animation without corrupting it, `ok()`/`fail()` finishers that leave a persistent themed success/failure line, and a `shown` property reporting whether a frame was ever drawn (to gate those finishers, which otherwise write unconditionally). Ships a 90-entry `SPINNERS` catalog of animations ported from cli-spinners, each a `SpinnerPreset` (frames plus tuned interval) selectable via `spinner=`.
- Add a `spinner` demo subcommand to the `click-extra` CLI: on an interactive terminal it animates a tour of catalog spinners (each held two to three seconds — three full cycles, floored and capped — or one full cycle when that alone runs longer), and `--table` prints them as a reference table (name, frame preview, interval, and per-spinner tour dwell time). A curated selection is shown by default; `--all`, `--random N` and `--select name1,name2` pick what to show.
...
- The `click:run` and `click:tree` Sphinx directives now capture executed CLIs at the file-descriptor level by default (Click `8.4`+), rendering commands that write through `sys.stdout.fileno()` instead of aborting the build; set the new `click_extra_run_capture` `conf.py` value to `"sys"` to opt out.
- `--jobs` (`JobsOption`) now accepts the keywords `auto` (one fewer than available CPUs) and `max` (all CPUs) on top of an integer, and its displayed default is `auto` instead of a machine-specific number. A value of `0` disables parallelism and runs sequentially.
