...
- Add a `click_extra_manpages` Sphinx configuration value (and the matching `click_extra.sphinx.manpages` module). Projects that already enable the `click_extra.sphinx` extension can declare one or more `{"script", "prog_name", "output_dir", "render_html"}` entries to emit the corresponding Click command tree(s) as roff `.1` files into `<outdir>/<output_dir>/` on every HTML build. Each `script` is resolved with the same scanner the `click-extra man` CLI uses (console-script entry point, `module:function`, `.py` path, or module name); only HTML-family builders (`html`, `dirhtml`, `singlehtml`) trigger the hook. When `mandoc` or `groff` is on `PATH`, every `.1` also gets a browser-viewable `.html` sibling so Sphinx's `:manpage:` role (driven by the standard `manpages_url` config) resolves to a readable page rather than a raw roff download. Set `render_html: False` per entry to suppress the HTML pass. The hook stays silent when the config is empty or absent.
- Add a `click-extra-manpages` Sphinx directive that renders a bullet list of links to every man page declared in `click_extra_manpages`. URLs are computed relative to the enclosing document, so the directive works at any depth in the doc tree. Empty config renders nothing.
