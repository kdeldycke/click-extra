...
- Add a `--tree` flag to the default option set: it prints the whole hierarchy of nested subcommands as a tree, then exits. Each node carries the command name, its aliases, its operand metavars (like the usage line) and its one-line description, column-aligned and wrapped at the terminal width with the rail running through. Subcommands are discovered dynamically, hidden ones are skipped and deprecated commands carry their marker. Names, aliases and metavars reuse the help-screen theme slots; the box-drawing rail degrades to pure ASCII under `--accessible`. The new `click_extra.tree` module exposes the `render_command_tree()` core, the `TreeOption` class and the `@tree_option` decorator.
- Add a `--tree` mode to `click-extra wrap`: prints the subcommand tree of any foreign Click CLI without running it, with the same target resolution and subcommand drilling as `--show-params` and `--man`.
- Help screens now style subcommand aliases with the `alias` theme slot and their parenthetical punctuation with `alias_secondary`, routed through Cloup's own alias formatter. Both slots were defined by every built-in theme but never consumed by the terminal rendering pipeline: alias words used to take the `subcommand` slot and the punctuation stayed plain. The `--tree` view renders aliases through the same formatter, so both outputs match in every theme.
- Table formats able to express styles natively (`html`, `unsafehtml`, `mediawiki`, `textile`, `jira` and the four `latex*` variants) now translate the ANSI codes carried by cells and headers to the format's own styling markup (inline-CSS `<span>`s, `%{...}` spans, `{color:...}` macros, `xcolor` LaTeX macros) instead of stripping them. Disable colors (`--no-color`, `NO_COLOR`, ...) to restore plain markup. Other markup formats keep stripping ANSI codes; the format-by-format verdict is documented in the table page.
- An explicit `--color` no longer injects raw ANSI codes into the markup formats supporting native styling: it renders the same translated markup as the default. The raw passthrough escape hatch remains for markup formats without styling support.
...
- Add `split_ansi()` and `render_ansi()` to `click_extra.styling`: a stateful ANSI SGR stream tokenizer yielding `(Style, text)` runs, and a driver re-rendering those runs through a markup emitter.
- Add the `ansi_to_html()`, `ansi_to_jira()`, `ansi_to_latex()` and `ansi_to_textile()` converters to `click_extra.styling`.
...
- Unify the palettes materializing the 16 ANSI colors as concrete RGB: `Style` CSS output and contrast math no longer use VGA values, and the first 16 slots of the 256-color indexed palette are no longer hard-coded to xterm's system colors. Both now share the palette the documentation renders terminal sessions with (Tango-flavored normal colors, iTerm2 bright variants). The 8 base named colors still pass through as CSS keywords in markup output.
- Add `run_cli(args)` to `click_extra.execution`: a `subprocess.run` work-alike for CLI-wrapping tools that logs the invocation up front (the copy-pasteable `$ ENV=value command` prompt line, at a configurable level defaulting to `INFO`) and streams the child's output to the logger line by line as it is produced (at `DEBUG`), instead of holding it back until the child exits. Mirrors `subprocess.run`'s contract: returns a `CompletedProcess`, raises `TimeoutExpired` (with the partial capture attached) after killing the child and, on Windows, its whole process tree.
- `run_cli`'s per-call `label` tags each streamed record with a `label` attribute, which the default `Formatter` renders glued to the level name and styled like an invoked command: `debug:mas: Warning: ...`. A foreign formatter can render `record.label` its own way.
- `format_cli_prompt` now styles each token family with the theme slot it holds elsewhere in a CLI's output: a dim `bracket` prompt sigil, `envvar`-name/`default`-value assignment pairs, the program's binary name as an `invoked_command` (directory plain), `option`-styled flags, plain arguments. `run_cli`'s spawn trace renders the binary path the same way.
- The binary-path renderer behind those lines is public API: `highlight_bin_name(program)` styles a path's binary name and leaves its directory plain, so wrapping tools can reuse the exact same rendering in their own output (tables, availability reports).
- `select_row` accepts any `Mapping`, not just `dict`, so read-only row shapes project without a copy.
- Add the `Duration` parameter type to `click_extra.types`: parses a friendly duration (`7 days`, `12h`), an ISO 8601 duration (`P7D`, `PT12H`) or an RFC 3339 timestamp (converted to its age at parse time) into a `datetime.timedelta`. Calendar units (months, years) are rejected as ambiguous. A zero duration, an empty value or a future timestamp parse to `None`, which cutoff options (cooldowns, timeouts, retention windows) read as "no cutoff".
- One `--sort-by` can now serve subcommands rendering heterogeneous tables. `SortByOption` accepts bare column IDs, declaring a sortable-field vocabulary untied to any single table layout: declared on a group, the selection is published on the shared context and resolved per table. `print_table()` and `render_table()` headers now accept `ColumnSpec` instances and `(label, column_id)` pairs alongside plain labels: rows sort by the selected fields the table carries (remaining columns breaking ties left to right) and keep their original order when it carries none. The per-table key builder is public API: `column_sort_key()`.
- An explicit empty `default=()` on `SortByOption` now declares no default sort, so bare invocations keep the original row order. Only an omitted / `None` default derives the first sortable column.
- Add the `isolated_app_dir` fixture to `click_extra.pytest`: repoints `click.get_app_dir`-based configuration discovery at a fresh per-test directory, so in-process CLI invocations never read the host's real configuration folder. Complements the `runner` fixture's `HOME` pinning, which only covers tests requesting that fixture, and leaves `HOME` untouched. The fixture returns the directory, so a test can plant a configuration file in it to exercise the default discovery.
- `--color`/`--no-color` now reach output produced from background threads: the color options mirror their resolved tri-state process-wide (`invocation_color()`), and `StreamHandler` consults it instead of `click.echo`'s thread-local context lookup, so the subprocess lines `run_cli` streams from its reader threads no longer keep their ANSI codes under `--no-color` on a terminal.
- `Formatter` no longer mutates a record's `levelname` in place: a record formatted several times (multiple handlers, captured then re-rendered) no longer accumulates styling or glued labels.
- Add `install_interrupt_handler(ctx)` and `terminate_live_processes()` to `click_extra.execution`: the first Ctrl+C now `SIGTERM`s every subprocess spawned through `run_cli`, so a concurrent fan-out aborts promptly instead of hanging on children that survived the terminal's process-group `SIGINT`.
- Move `args_cleanup`, `format_cli_prompt`, `PROMPT` and `INDENT` from `click_extra.testing` to `click_extra.execution`: they serialize and disclose subprocess invocations for production wrappers, not just tests. `click_extra.testing` still consumes the first three, so their historical import path keeps working; `PROMPT` moves outright.
...
- `test-suite` now runs its target binaries through `run_cli`: each invocation is disclosed as a themed prompt line and its output streams live to `DEBUG` logs, making long-running cases observable while they run.
