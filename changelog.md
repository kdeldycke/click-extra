...
- Add a `Spinner` widget: an indeterminate, thread-animated terminal spinner for long-running blocking work where `click.progressbar` does not fit. Auto-disabled when its output stream is not a TTY. Usable as a context manager or a decorator (with or without parentheses), with a live-updatable label, custom frames, reverse rotation, `Style`-based coloring, an opt-in terminal-bell `beep` on stop, an elapsed-time `timer`, an `echo()` method to print above the animation without corrupting it, and `ok()`/`fail()` finishers that leave a persistent themed success/failure line. Ships a 90-entry `SPINNERS` catalog of animations ported from cli-spinners / ora, each a `SpinnerPreset` (frames plus tuned interval) selectable via `spinner=`.
