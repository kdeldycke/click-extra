...
## [`8.2.0.dev0` (unreleased)](https://github.com/kdeldycke/click-extra/compare/v8.1.4...main)
...
- Add the always-on `matrix` Sphinx directive to `click_extra.sphinx`, which renders a package's release compatibility matrix for a given axis: `{matrix} python` (interpreter versions from classifiers or `requires-python`) or `{matrix} <distribution>` (a dependency's accepted versions from its requirement specifier, with auto-derived columns and an optional `Spec` column). The table is embedded in the block and rendered without git access at build time. Options: `:package:`, `:path:`, `:version-floor:`, `:tag-pattern:`, `:python-floor:`, `:show-spec:`. Needs no `click_extra_enable_exec_directives` opt-in. A `<!-- matrix <axis> -->` … `<!-- matrix-end -->` comment-marker form embeds the raw table so it also renders as a real table on GitHub.
- Add the `click-extra refresh-directives` command (and the `click_extra.sphinx.matrix.update_matrix_blocks` function it wraps) to regenerate the tables embedded in `{matrix}` directive blocks and `<!-- matrix … -->` marker regions from a project's git tags. It refreshes only existing blocks, fails gracefully when the sphinx extra is absent, and its `--check` mode flags stale tables in CI.
