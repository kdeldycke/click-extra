...
- **Breaking:** Move the help-screen highlighting engine out of `click_extra.colorize` into a new `click_extra.highlight` module: `HelpFormatter`, `HelpKeywords`, `highlight`, and the internal `_HelpColorsMixin`/`_escape_for_help_screen`. The `colorize` module now hosts only the `--color`/`--no-color` and color-environment resolution it is named for. The public names re-exported at the `click_extra` package root are unchanged; update any deeper `from click_extra.colorize import HelpFormatter` to `from click_extra.highlight import HelpFormatter`.
