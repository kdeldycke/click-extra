...
{mod}`click_extra.sphinx.myst_docstrings` fills the same gap without replacing anything: it converts MyST to reST on the way in, so `sphinx.ext.autodoc` keeps doing the work, and `sphinx_autodoc_typehints`, `autodoc_default_options`, `autoclass_content` and everything else built on autodoc keep working as they are. Cross-references, inline code, links, fenced directives, plain code blocks and footnotes are all handled, and a docstring already written in reST passes through untouched. See [Comparison with `sphinx-autodoc2`](#comparison-with-sphinx-autodoc2) for a detailed feature matrix.
