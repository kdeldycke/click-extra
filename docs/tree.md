...
The tree is a human rendering only. [Machine-readable hierarchies](machine-readable.md) are covered elsewhere: `--help-format json-full` walks the whole tree as JSON, `--params --table-format json` serializes every parameter with its dotted path, and [`--help-format carapace`](carapace.md) emits the command-and-flag tree as YAML.
