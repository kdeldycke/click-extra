...
A committed capture is rewritten on every build, which is what keeps it from drifting away from the CLI. A recording fights that: its durations are wall-clock, so a busier machine times the same frames a few milliseconds differently and the file changes with nothing behind it. Two mechanisms answer the two ways that happens.

`quantize` rounds each duration onto a grid coarser than the jitter, so both runs land on the same numbers:

```python
from click_extra.recording import quantize

frames = quantize(record_command(("kettle", "boil"), duration=5.0))
```

That settles jitter and cannot settle a frame the scheduler dropped, which leaves a shorter recording of the same frames. So an animated capture also states what it is, on a line beside its generator tag:

```xml
<!-- @generated by Click Extra 9.0.0 -->
<!-- @recording frames=8 period=0.64s digest=2b5a410759b29890 -->
```

The digest covers the frames a cycle holds and the beat it holds them on, not their order or their count. A build compares that line against the committed one and, when the two match, **leaves the file exactly as it found it**. A dropped frame therefore costs nothing, and a command whose output really changed still writes.

Reporting rather than rewriting is deliberate. A build that rewrote on every timing wobble would turn an ordinary rebuild into a dirty working tree, and a dirty tree nobody caused is a signal people learn to ignore.

```{note}
The declared form of [`:screenshot-animate:`](sphinx.md#committed-captures) needs none of this: frames composed from a spinner are the same on every build, so the asset is byte-identical without a grid or a fingerprint. Both are for the recorded form.
```
