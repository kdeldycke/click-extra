...
Neither property is redundant. `visibility` alone satisfies a browser, `librsvg` and Quick Look's thumbnailer, and a git client's SVG diff view ignored it outright and drew all sixty-one frames stacked on each other; adding `opacity` fixed that one. So each covers a reader the other misses, and the animation restores both together rather than picking a side.
