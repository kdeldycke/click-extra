...
If you only ever use the decorators click-extra ships out of the box, this page is not for you: `@command`, `@group`, `@option` and the rest work as their Click and Cloup counterparts do. It is for downstream code wiring its own `@my_option` decorator into the same factory.

That factory wraps cloup's originals with three behaviors, which is what a decorator built on it inherits:

1. **Subclass-enforced `cls=`**: every `@command` / `@group` always yields a `Command` / `Group` (or a user-supplied subclass thereof), so click-extra's machinery (config loading, theme registry, `--params` introspection) can't be silently bypassed by passing a vanilla `click.Command` class.
