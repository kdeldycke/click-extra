...
| [`solarized-dark`](#solarized-dark) | 24-bit RGB           | Warm-toned dark theme with selective accent contrast.                |
...
`````{tab-item} solarized-dark
```{click:run}
result = invoke(weather, args=["--theme", "solarized-dark", "--help"])
assert result.exit_code == 0
```
`````
