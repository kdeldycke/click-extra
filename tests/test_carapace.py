...
# -- CLI surface (wrap --help-format carapace) --------------------------------------------
...
def test_wrap_carapace_emits_valid_spec(runner, greet_script):
    result = runner.invoke(
        demo, ["wrap", "--help-format", "carapace", greet_script], color=False
    )
    ...
    assert "\n# $ click-extra wrap --help-format carapace -- " in result.stdout
    assert f"# Documentation: {CARAPACE_DOCS_URL}\n" in result.stdout
...
def test_wrap_carapace_install(runner, greet_script, tmp_path, monkeypatch):
    ...
    result = runner.invoke(
        demo,
        ["wrap", "--help-format", "carapace", "--install", greet_script],
        color=False,
    )
    assert result.exit_code == 0
...
def test_wrap_carapace_mutually_exclusive(runner, greet_script, extra_flags, message):
    result = runner.invoke(
        demo,
        ["wrap", "--help-format", "carapace", *extra_flags, greet_script],
        color=False,
    )
...
def test_wrap_install_requires_an_installable_format(runner, greet_script):
    """A destination only means something for a rendering someone installs."""
